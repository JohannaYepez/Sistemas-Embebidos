...
float prom;
float dist;
const int k = 3; //declarar k
float dist_menor[2][k]; //declarar matriz para almacenar datos
int i = 0;
int j = 0;
...
void loop() {
  for (; fila < 90; fila++) { //leer matriz de 90 datos
    ...
    if (dist < dist_menor[0][k - 1]) { //comparar distancias
      dist_menor[0][k - 1] = dist; //almacenar distancia
       dist_menor[1][k - 1] = matriz[fila][4]; //almacenar etiqueta
    }
    for (; i < k; i++) { //ordenar matriz
      for (j = i + 1; j < k; j++) {
        if (num_etiquetas[0][i]  > dist_menor[0][j]) {
          aux = dist_menor[0][i];
